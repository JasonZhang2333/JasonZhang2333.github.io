...
2. *损失函数*，搭建模型使损失函数达到最小值，定义损失函数如下：

       $$LL=\sum_i\vert y_i-\hat{y_i}\vert$$

       这一损失函数对应着Least Absolute Deviations Regression，但上式并不处处可导，数学求最值比较麻烦，所以重新定义如下：

       $$LL=\sum_i(y_i-\hat{y_i})^2$$

       这一损失函数对应着Least Squares Regression。
