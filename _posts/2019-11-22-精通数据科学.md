...
- $a,b$是模型的参数，表示生产一个玩偶的变动成本和固定成本，$\varepsilon_i$被称为噪声想，表示没有捕捉到的随机成本，$\varepsilon_i\sim N(0,\sigma^2)$，这里假设$\{\varepsilon_i\}$之间相互独立，而且$\{x_i\}$和$\{\varepsilon_i\}$之间也相互独立
- 根据已知条件总结，$\{y_i\}$之间相互独立，且满足条件概率如下：
$$P(y_i\vert a,b,x_i,\sigma^2)\sim N(ax_i+b,\sigma^2)$$

2. *估计参数*
- 根据分析，$\{y_i\}$之间相互独立，得到$\{y_i\}$出现的联合概率如下所示，这个概率被称为模型的似然函数(likelihood function，通常记为$L$)：
...
- $\{y_i\}$出现的概率越大越好，所以使似然函数最大的参数将是参数估计的最佳选择，这一方法被称为最大似然估计(Maximum Likelihood Estimation, MLE)，参数$(a,b)$的估计值$(\hat{a},\hat{b})$如下：
